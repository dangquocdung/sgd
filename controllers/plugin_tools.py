...
def define_tables():
	for table in ['folder','dtable','rtable','dfield','tablefield','dcontent','box','rtable','metadata','dmeta']:
		cms.define_table(table)
	rows = db(db.dtable).select(orderby=db.dtable.display_order)	
	for row in rows:
		cms.define_table(row.name)
...
def select_table():
	...
	if table_exports <> []: 
		...
		elif request.args(0)=='delete': delete(table_exports)		
	return dict(tables=db.tables) 
...
def table2xls(table, query=None):
	...
	for field in fields:			
		...
		#if field['is_unique']==True: name += ',unique'
		lign1.append(field)
		lign2.append(name)
	sheet = [lign1,lign2]	
	i = 1
