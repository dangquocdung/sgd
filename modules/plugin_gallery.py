...
class Gallery():
	def __init__(self,**attr):
		...
		self.tablename = attr.get('tablename', current.request.vars.tablename or 'gallery')
		self.table_id = attr.get('table_id', current.request.vars.table_id or current.request.vars.uuid)
		self.attachment_id = attr.get('attachment_id', current.request.vars.attachment_id)
		self.table = attr.get('gallery','gallery')
		self.path = '%s/static/uploads/gallery/%s'%(current.request.folder, self.tablename)
	...
	def get_row(self):
		...
		T = current.T
		content = DIV(_id='gallery',_class=self.tablename) 
		attach = self.define_table()
		rows = db((attach.tablename==self.tablename)&(attach.table_id==self.table_id)).select()
		return rows
	...
	def view(self,crud=['delete'],type='table',length=10,edit_online=False):
		...
		if type=='ul': ul = UL(_class='gallery')
		...
		for row in rows:
			if str(row.filepath).startswith(self.table):
				...
				href=URL(r=request,c='static',f='uploads/%s/%s'%(self.tablename,row.filepath))
				name = A(row.name, _href=href)
			...
			if os.path.isfile(self.path+'/%s.pdf'%row.name):
				url = 'http://%s/%s/static/uploads/%s/%s.pdf'%(request.env.http_host,request.application,self.tablename,row.name)
			elif (icon in ['png','jpg','jpeg','bmp','gif']):
				url = 'http://%s/%s/static/uploads/%s/%s'%(request.env.http_host,request.application,self.tablename,row.filepath)
			else:
				url = 'http://%s/%s/static/uploads/%s/%s'%(request.env.http_host,request.application,self.tablename,row.filepath)
	...
	def update_name(self,name):
		try:
			db = self.db
			attach = self.define_table()			
			db(attach.id==self.attachment_id).update(name=name)	
		except: pass
	...
	def upload_data(self,fobject):
		...
		try:
			...
			print 'plugin_gallery module upload_data error: %s'%e
