...
class Attachment:
	def __init__(self,**attr):
		...
		self.path = '%s/uploads/documents'%(current.request.folder)
	...
	def upload(self):
		...
		if request.vars.qqfile:
			...
			self.db(attach.id==id).update(filesize=filesize)
	...
	def upload_data(self,fobject):
		...
		try:
			...
			id = table.insert(name=name,extension=extension,filepath=table.filepath.store(fobject.file,fobject.filename))
			filesize= (0.0+os.path.getsize(self.path+'/'+table(id).filepath))/1024
			name = name.replace(' ','_')
			...
			print 'plugin_doc module upload_data error: %s'%e
